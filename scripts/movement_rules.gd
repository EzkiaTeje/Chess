...
static func is_square_occupied(board_position: Vector2i, selected_piece: Piece, pieces: Array) -> bool:
	for piece in pieces:
...
static func is_square_occupied_by_rival(board_position: Vector2i, selected_piece: Piece, pieces: Array) -> bool:
	for piece in pieces:
		if piece == selected_piece:
			continue
...
static func is_square_occupied_by_ally(board_position: Vector2i, selected_piece: Piece, pieces: Array) -> bool:
	for piece in pieces:
		if piece == selected_piece:
			continue
		if Vector2i(piece.position / Globals.CELL_SIZE) == board_position and piece.piece_color == selected_piece.piece_color:
